...
def replace_row(currency_pair, index, bid, ask,lowest_spread):
    ...
    return [
        html.P(
            currency_pair, id=currency_pair, className="three-col"  air name
        ),
        html.P(
            new_row[1].round(5),  
            id=new_row[0] + "bid",
        ...
        html.P(
            ...
            style={"color": get_color(new_row[2], lowest_spread)},
        ),
        html.Div(
...
def cci_trace(df, ndays=5):
	...
	return trace
'''
def lowest_spread_trace(df, ndays=5):
	#print(df)
	lowest_spread = df['current_lowest_spread']
	trace = go.Scatter(x=df.index, y=lowest_spread, mode='lines+markers',showlegend=False, name="lowest_spread")
	return trace
